<script lang="ts">
  ...
  type PreviewNode =
    | { kind: 'folder'; path: string; name: string; depth: number; open: boolean; childCount: number }
    | {
        kind: 'file';
        path: string;
        name: string;
...
<div class="w-full overflow-hidden">
  {#each nodes as node (node.kind + ':' + node.path)}
    ...
    {:else}
      ...
      >
        <span
          class="grid size-2.5 shrink-0 place-items-center rounded-[3px] border border-border"
        ></span>
        {#if showIcons}
          <Icon icon={iconName} class="size-3 shrink-0" />
        {/if}
        {#if node.dirPrefix !== undefined}
          <span class="flex min-w-0 flex-1 items-center text-[10px]">{#if node.dirPrefix}<span class="min-w-0 truncate text-muted-foreground">{node.dirPrefix}</span>{/if}<span class="shrink-0">{node.name}</span></span>
        {:else}
