...
<Dialog.Root {open} onOpenChange={() => {}}>
  ...
  >
    <Dialog.Header class="flex flex-row items-start gap-2 space-y-0 border-b border-border p-4">
      ...
      <div class="flex-1">
        ...
        <Dialog.Description class="mt-1 text-xs">
          Pulling from origin produced conflicts. Open each file in your editor, resolve
          the markers, then mark it resolved.
          {#if isPullOnly}
    ...
    <Dialog.Footer class="flex flex-row items-center justify-between gap-2 border-t border-border p-3 sm:justify-between">
