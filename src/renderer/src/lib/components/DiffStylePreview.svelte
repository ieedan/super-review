...
`;
  ...
  function render(): void {
    ...
    try {
      ...
      console.log('[DiffStylePreview] render returned', result, 'shadowChildren?', container.shadowRoot?.childNodes.length);
    } catch (err) {
  ...
  onMount(() => {
    ...
    void ensureDiffHighlighter().then(() => {
      ...
      try {
        instance.rerender();
        console.log('[DiffStylePreview] post-rerender shadowChildren?', container?.shadowRoot?.childNodes.length);
      } catch (err) {
