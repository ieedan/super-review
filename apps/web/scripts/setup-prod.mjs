	...
	await step(
		...
		async () => {
			...
			describe([
				...
				'one fixed origin, so previews only really work against a stable alias.',
				'Vercel gives every branch one:',
				dim('  https://<project>-git-<branch>-<team>.vercel.app'),
				'',
				'Set the branch you actually test on, or skip and set SITE_URL by hand on',
				'the preview deployment when you need it. Per-commit preview URLs still',
				'build, they just will not complete an OAuth round trip.'
			]);
