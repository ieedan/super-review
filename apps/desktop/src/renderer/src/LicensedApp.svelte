<script lang="ts">
	...
	onMount(() => {
		// LicensedApp only mounts when licensed, so mounting IS the gate — call
		// full init here. The store skips if mainInitCompleted already.
		void actions.init();
...
<div class="flex h-screen w-full flex-col">
	...
	>
		...
		<main class="flex min-h-0 flex-1">
			{#if app.initializing}
				<!--
					Hold the pane blank until init resolves the persisted repo. Rendering
					the empty state here would flash it on every launch that has one.
				-->
			...
			{:else}
				...
				>
					...
					<!-- Statically collapsible (never a reactive constraint — see the layout
				     ...
				     `defaultSize` must be a STATIC literal: in paneforge it lives in the
				     same reactive `constraints` bag as minSize/maxSize, so tying it to
				     `commentsSidebarOpen` made every sidebar toggle reload the whole
				     group layout from storage (a visible relayout, not instant). It only
				     seeds the diff's first-ever width anyway; opening the comments pane
				     allocates from its own defaultSize, and the saved layout takes over
				     after that. It's a percentage but paneforge normalizes the seeds by
				     ratio, so a value >100 just means "as wide as possible" — it pins the
				     file sidebar to its 330px minimum (SIDEBAR_MIN_PX) and gives the diff
				     everything else. -->
					<Resizable.Pane
						bind:this={mainPane}
						id="main-pane"
