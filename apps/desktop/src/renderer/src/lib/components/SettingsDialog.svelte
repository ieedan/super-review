<script lang="ts">
	...
	import type { Accent, EditorKind, TerminalKind, ViewMode } from '@shared/types';
				...
				{:else if activeTab === 'appearance'}
					<section class="space-y-6">
						...
						<div>
							...
							<p class="mt-1 text-xs text-muted-foreground">
								Enable enter/exit and hover transitions on menus, dialogs, tooltips, and other UI.
								Off by default for a snappier, motion-free feel.
							</p>
				...
				{:else if activeTab === 'behavior'}
					<section class="space-y-6">
						<div>
							...
							<p class="mt-1 text-xs text-muted-foreground">
								Choose what happens when you move the keyboard cursor onto a file with the arrow
								keys in the sidebar.
							</p>
						...
						<div>
							...
							<p class="mt-1 text-xs text-muted-foreground">
								Files matching these glob patterns have their diffs hidden behind a "Load diff"
								button by default — lock files, build outputs, etc. A pattern with no slash matches
								the file name anywhere (e.g. <code>*.lock</code>); one with a slash matches the full
								path (e.g. <code>dist/**</code>).
							</p>
