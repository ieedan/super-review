<script lang="ts">
	...
	import { actions, app, allBranchChangesSeen } from '$lib/store.svelte';
	...
	// The "search files (command palette)" shortcut that toggles this palette is
	...
	const availablePathWidth = $derived.by(() => {
		...
		return Math.max(0, listWidth - groupX - itemX - checkbox - iconAndGap - gaps - margin);
	});
</script>
		...
		<Command.Group>
			{#each app.changedFiles as file (file.path)}
				...
				{@const displayPrefix = parts.dir
					...
					: ''}
				<Command.Item value={file.path} onSelect={() => choose(file.path)} class="gap-2">
               ...
               the 14px circle and oversize the language icon. -->
					<span
						class={cn(
							...
							isSeen ? 'text-success-foreground border-success bg-success' : 'border-border'
						)}
               ...
               The prefix is pre-truncated to fit `availablePathWidth`. -->
					<span
						class={cn(
							...
							isSeen && 'text-muted-foreground line-through'
						)}
						>{#if displayPrefix}<span class="text-muted-foreground">{displayPrefix}</span>{/if}<span
							class="shrink-0">{parts.name}</span
						></span
					>
				</Command.Item>
			{/each}
