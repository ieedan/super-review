...
// Force links to open in a new context and strip referrer/opener, and never let
// a `javascript:` URL slip through (DOMPurify drops those too). Registered
// lazily on first render: DOMPurify's default export is only a usable instance
// in the browser (it needs a `window`), so touching it at module load would
// crash under SSR/prerender, where this module can be pulled in transitively.
let purifyHookReady = false;
function ensurePurifyHook(): void {
	if (purifyHookReady) return;
	purifyHookReady = true;
