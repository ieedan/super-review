...
<!--
  ...
  light and dark (Codex, opencode, Copilot) draw with `currentColor` and inherit
  ...
  the existing currentColor icon. Anything without a bundled logo (other) falls
  back to a generic agent icon.
-->
...
{:else if harness === 'copilot'}
	<!-- Copilot's mark is a single-color glyph; draw it with `currentColor` +
	     `text-foreground` so it themes like Codex/opencode (the source art ships
	     white, which would vanish in light mode). -->
	<svg
		xmlns="http://www.w3.org/2000/svg"
		width={size}
		height={size}
