...
import { CENTS_PER_UNIT, sum } from './money.js';
...
export function settleUp(balances: Balances): Transfer[] {
	...
	while (d < debtors.length && c < creditors.length) {
		...
		// leaves the other carrying the difference into the next pass.
		const owed = Math.min(-debtor.amount, creditor.amount);

		// Nobody can hand over a fraction of a cent, so go out to major units,
		// round there, and come back to something payable.
		const units = Math.round(owed) / CENTS_PER_UNIT;
		const amount = units * CENTS_PER_UNIT;
