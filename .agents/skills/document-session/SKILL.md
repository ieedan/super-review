...
description: Document changes you made in your session in a Super Review tour to make them easier for users to parse and understand, and read & resolve the reviewer's inline review comments - all via the Super Review CLI.
---
...
to document the fixes under one `--key` -> `comment resolve <id> --session
