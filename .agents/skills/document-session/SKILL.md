...
description: Document the changes you made as a guided super-review session — a tour that groups related files into explained steps — and read & resolve the reviewer's inline comments, all via the super-review CLI. Use after completing edits in a repo the user reviews with super-review, or when picking up review comments to address.
---
...
- **List every file you changed, and only those.** When you pass a tour, the
  session captures *only* the files the tour references — that's the point: the
  ...
  leave out is *not* captured; an unchanged file you list is dropped (the CLI
  warns about it).
...
A good loop: `comment list --unresolved` → fix each comment → `session save` to
document the fixes under one `--key` → `comment resolve <id> --session "<key>"`
for every comment you addressed. The reviewer then sees each comment resolved,
with a link to the session explaining exactly how.
